<template>
  ...
  <v-container v-if="faktury">
    <v-row>
      <v-col cols="auto">
        <v-select class="selector" label="Poskytovatel" v-model="provider_id" :items="entities" item-title="abbreviation"
          item-value="id"></v-select>
      </v-col>
      <v-col cols="auto">
        <v-select class="selector" label="Odběratel" v-model="client_id" :items="entities" item-title="abbreviation"
          item-value="id"></v-select>
      </v-col>
      <v-spacer></v-spacer>
      <v-col cols="auto">
      ...
      <v-row v-for="invoice in existing_invoices" :key="invoice.name" justify="space-between">
        <v-col cols="9" align-self="center">
          <v-row no-gutters justify="start">
            <v-col cols="2"><v-icon icon="mdi-file-pdf-box"></v-icon></v-col>
